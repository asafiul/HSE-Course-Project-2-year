...
int main() {
    ...
    vector<int> left(N, -1);
    for (int i = 0; i < n; ++i) {
        if (left[cpvs[i].first] != -1) {
            ptr[i] = ptr[left[cpvs[i].first]];
        }
        left[cpvs[i].first] = i;
    }
    ...
    for (int i = 0; i < n; ++i) {
        ...
        auto getind = [&](const S& s) {
            if (s.sv % sm != 0 || s.sv / sm == 0 || left[s.sv / sm] == left[s.sv / sm - 1]) {
            ...
            //     show(r0);
            // }
            int ind = left[s.sv / sm];
            show(s);
            show(s.sv, sm, s.sv/sm, ind);;
            assert(checkpoints[ind].sv == s.sv);
            if (cmp(checkpoints[ind], s)) {
                // assert(r0 == ind + 1);
                return ind + 1;
            }
            int& r = ptr[ind];
            show(ind, r);
            if (cmp(s, checkpoints[r])) {
                // assert(r0 == r);
                return r;
            }
            while (r < ind && cmp(checkpoints[r], s)) {
                ++r;
            }
            show(ind, r);
            // assert(r0 == r);
